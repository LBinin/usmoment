...
import type { ApiRow, TypeSection } from "./types";

export function businessKeyboardPropsRows(locale: Locale): ApiRow[] {
  ...
  return [
    ...
    row("topAccessory", "React.ReactNode", false, zh ? "键盘行上方的结构性扩展槽；UI 仅提供占位容器，具体内容和业务样式由调用方或 Kit 注入。" : "Structural slot above keyboard rows. UI only provides the placeholder container; callers or Kits own content and business styling."),
    row("bodyOverlay", "React.ReactNode", false, zh ? "覆盖在键盘按键区域上的结构性槽位；UI 只保证定位容器，业务内容、样式和动画由调用方或 Kit 注入。" : "Structural overlay slot above the keyboard key body. UI only provides the positioned container; callers or Kits own content, styling, and animation."),
...
export function calcDisplayPropsRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function popupPropsRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function popupTypeSections(locale: Locale): TypeSection[] {
  const zh = isZh(locale);
...
export function fullscreenOptionListPropsRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function fullscreenOptionListTypeSections(locale: Locale): TypeSection[] {
  const zh = isZh(locale);
...
export function accountingDisplayPropsRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function accountingCalculatorPopupPropsRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function accountingCategorySelectorPropsRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function accountingCategorySelectorTypeSections(locale: Locale): TypeSection[] {
  const zh = isZh(locale);
...
export function accountingCalculatorPropsRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
  ...
  return [
    ...
    row("topAccessoryItems", "AccountingCalculatorTopAccessoryItem[]", false, zh ? "渲染键盘上方横向滚动的 Kit 胶囊项；默认不内置任何项。" : "Renders Kit-owned horizontally scrollable capsule items above the keyboard. No items are included by default."),
    row("renderTopAccessoryItem", "(input) => React.ReactNode", false, zh ? "自定义单个 top accessory item 的内容；横向滚动容器仍由 Kit 控制。" : "Custom renderer for each top accessory item while the Kit still owns the horizontal container."),
    row("renderTopAccessoryActionPanel", "({ item, close }) => React.ReactNode", false, zh ? "点击 top accessory 胶囊后渲染按键区域上的操作面板；Kit 只提供激活状态、动画和关闭回调，具体内容由调用方传入。" : "Renders an operation panel over the key body after a top accessory capsule is clicked. The Kit only owns activation, animation, and close wiring; callers provide the content."),
...
export function businessKeyboardCoreApiRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function expressionEngineApiRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function selectionStateCoreApiRows(locale: Locale): ApiRow[] {
  const zh = isZh(locale);
...
export function businessKeyboardTypeSections(locale: Locale): TypeSection[] {
  const zh = isZh(locale);
