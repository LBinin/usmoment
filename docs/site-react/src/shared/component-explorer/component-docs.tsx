...
function accountingCalculatorPropsRows(locale: Locale): ApiRow[] {
  ...
  return [
    ...
    row("renderTopAccessoryPanel", "({ item, close }) => React.ReactNode", false, zh ? "点击 top accessory 胶囊后渲染按键区域上的操作面板；Kit 只提供激活状态、动画和关闭回调，具体内容由调用方传入。" : "Renders an operation panel over the key body after a top accessory capsule is clicked. The Kit only owns activation, animation, and close wiring; callers provide the content."),
